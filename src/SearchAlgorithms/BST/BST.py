...
#
#   Rev 2: 12/23/2013
#       1) Provide 3 tree generation options:
#           a) manually create tree,
#           b) generate random unbalanced tree,
#           c) create balanced tree.
#       2) Improve draw method to include left, right, and middle "invisible" nodes and intelligent edge weights,
#            now all edges in same direction are parallel,
#            and every node of the same depth is displayed at the same depth.
...
#       3) Add new binaryTree.delete() method to delete a node, and helper function binaryTree.children_count()
#           to return node's number of children
#       4) Add new binaryTree.print_tree() method to list node values in sorted order to standard output
#       5) Improve insert recursive binaryTree.insert() method:
#           a) detect when node already present,
#           b) return inserted node object on success
#       
loop_counter = 0
